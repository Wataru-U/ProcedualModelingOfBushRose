...
class ShootParameter(BranchParameter) :
    def __init__(self,thickness = 0.75,sl = 5,smooth = 1,ph = 40,pw = 40,sn = 4) :
...
class OldBranchParameter(BranchParameter) :
    def __init__(self,thickness = 0.5,sl = 3,sn = 7,smooth = 1,ph = 80,pw = 80) :
...
class FlowerBranchParameter(BranchParameter) :
    def __init__(self,thickness = 0.3,sl = 3,sn = 9,smooth = 1,fnum = 1,fneck = 0,g = 0) :
        super().__init__(thickness,sl,sn,smooth)
        #　花数
        self.flowerNum = fnum
        #　花首下がるかどうか 0~1
...
# ウィンドウ込み
class RoseParameter:
    def __init__(self,smoothness = 16,inclination = math.pi / 3, strength = 10,exponent = 1,lh = 0.1,prob = 5,b = OldBranchParameter(),sb = ShootParameter(),fb = FlowerBranchParameter()) :
